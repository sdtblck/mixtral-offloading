...
def unpack_4bit_u8_common(W_q: torch.Tensor):
    ...
    W_q = W_q.to(torch.uint8)
    result = torch.empty(
        [2 * height] + list(W_q.shape[1:]), dtype=torch.uint8, device=W_q.device
    )
    result[::2, ...] = W_q >> 4
...
def unpack_2bit_u8_common(W_q: torch.Tensor):
    ...
    height = W_q.size(0)
    result = torch.empty(
        [4 * height] + list(W_q.shape[1:]), dtype=torch.uint8, device=W_q.device
    )
    result[::4, ...] = (W_q >> 6) & 0b11
...
def unpack_3bit_i32_common(W_q: torch.Tensor):
    ...
    result = torch.empty(
        [10 * height] + list(W_q.shape[1:]), dtype=torch.uint8, device=W_q.device
    )
